<script lang="ts">
	...
	function applyRepulsionForce(draggedNode: Node, depth: number = 0, forceMultiplier: number = 1) {
		// Prevent infinite recursion and limit computational cost
		...
		nodes.forEach(otherNode => {
			...
			if (distance < repelDistance && distance > 0) {
				...
				otherNode.x = Math.max(nodeRadius, Math.min(canvasWidth - nodeRadius, otherNode.x));
				otherNode.y = Math.max(nodeRadius, Math.min(canvasHeight - nodeRadius, otherNode.y));
